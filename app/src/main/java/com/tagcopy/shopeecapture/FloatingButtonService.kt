...
class FloatingButtonService : Service() {
    ...
    private fun onDetectButtonTapped() {
        ...
        // 【階段2測試，暫時加的】借用偵測按鈕順便測試影片登記進媒體庫功能+候選商品掃描+節點樹診斷+上架自動化（1筆），
        // 結果看debug log，不影響下面原本「顯示目前套件名稱」的功能。
        // 這個上架自動化整個做完之後，這幾行連同ShopeeAccessibilityService.kt裡的
        // testMediaStoreRegistration()/testScanUploadCandidates()/dumpCurrentNodeTree()/testUploadAutomation()都可以一起移除。
